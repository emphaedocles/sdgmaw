...
function events.CalcDamageToPlayer(t)
    ...
    SetCharDamText(t)
--    if t.Result > 0 and source and source.Monster then
--        -- add to combat log
--        local monName = "??"
--        if (source.Monster) then
--            if source.Monster.NameId > 0 then
--                monName = Game.PlaceMonTxt[source.Monster.NameId]
--            else
--                monName = source.Monster.Name
--            end
--        end
--        if not monName then monName = "??" end

--        --        AddCombatLog(StrColor(150, 0, 0, string.format("%s hits %s for %s damage!", monName, t.Player.Name, t.Result)))
--        AddCombatLog(monName .. " damaged (2) " .. t.Player.Name .. " " .. StrColor(255, 0, 0, shortenNumber(t.Result, 4, true)))
--    end
end
...
function events.CalcDamageToMonster(t)
    ...
    if data and data.Player and t.Result > 0 then
        ...
        local fullHP = GetMaxHP(pl) or 0
        -- if player is disintegrated
        ...
        local baselineHeal = t.Result / refHP * fullHP
        -- basically dealing 100% of monster B HP as damage heals you by 100%
        ...
        if manaLeechLeg then
            ...
            totalHeal = totalHeal / fullHP
            -- remove mana portion
            fullHP = GetMaxHP(pl)
            totalHeal = fullHP * totalHeal
            -- add life portion
        end
...
-- 			end
-- 			if castedAoe then
-- 				msg=string.format("%s hits for a total of %s points!%s (aoe)", name, msgTxt, critMessage)
...
function events.CalcDamageToMonster(t)
    ...
    if data and data.Player then
        ...
        local locDam = math.ceil(t.Result * divide)
        MSGdamage = MSGdamage or 0
        MSGdamage = MSGdamage + locDam
        local msgTxt = MSGdamage
        msgTxt = shortenNumber(msgTxt, 4, true)
        -- msgTxt =msgTxt .. " (" .. locDam .. ")"
        ...
        monName = monName .. "(" .. t.Monster.NameId .. ")"

        local msgLoc = StrColor(255, 32, 32, locDam .. " (S: " .. msgTxt .. ")")
        ...
        RunNextTick( function()
            if id <= Map.Monsters.High and MSGdamage > 0 then
                if shoot == "shoots" then
                    msg = string.format("%s shoots %s for %s points!%s", plName, monName, msgTxt, critMessage)
                    clog = string.format("%s shoots %s for %s points!%s", plName, monName, msgLoc, critMessage)
                else
                    msg = string.format("%s hits %s for %s points!%s", plName, monName, msgTxt, critMessage)
                    clog = string.format("%s hits %s for %s points!%s", plName, monName, msgLoc, critMessage)
                end
                if t.Monster.HP <= 0 then
                    msg = string.format("%s inflicts %s points killing %s!%s", plName, msgTxt, monName, critMessage)
                    clog = string.format("%s inflicts %s points killing %s!%s", plName, msgLoc, monName, critMessage)
                end
                if castedAoe then
                    msg = string.format("%s hits for a total of %s points!%s", plName, msgTxt, critMessage)
                    clog = string.format("%s hits for a total of %s points!%s", plName, msgLoc, critMessage)
                end
                Game.ShowStatusText(msg)

                AddCombatLog(clog)
                SDGAddDPSTracking(plName, locDam)
