...
function recalculateMawMonster()
    ...
    if mapvars.boosted == nil then
        -- needed for retrocompatibility, otherwise unique monsters from old saves gets bosted again
...
function events.MonsterKillExp(t)
    ...
    for i = 0, Party.High do
        if Party[i].Dead == 0 and Party[i].Eradicated == 0 then
            ...
            -- accounts for the cases which you want to level a low lvl character
            --[[
    ...
    for i = 1, 651 do
        ...
        if Game.freeProgression == false and not mapvars.mapAffixes then
            ...
            if not horizontalMaps[name] then
                ...
                -- necessary to avoid making too much difference between monster tier
        ...
        if Map.Name == "d42.blv" then
            ...
            bolsterLevel = base.Level * horizontalMultiplier
            flattener =(base.Level - LevelB) * horizontalMultiplier * 0.8
            -- necessary to avoid making too much difference between monster tier
        ...
        -- experience
        local lvlBase = math.max(basetable[i].Level, totalLevel[i] / 3)
        -- added totalLevel/3 because of mapping
    ...
    if Map.IndoorOrOutdoor == 1 and mapvars.monsterMap and mapvars.monsterMap.cleared == false then
        ...
        for i = 0, #mapvars.monsterMap do
            ...
            if mon and table.find(playerControlledMonsters, i) or(mon and mon.NameId >= 220 and mon.NameId <= 300) then
                ...
                if mon and old and old.respawn and(mon.AIState == const.AIState.Removed or mon.AIState == const.AIState.Dead) then
                    -- no unique monsters respawn
                    mon.HP = mon.FullHP
...
local function _H(s)
    local t = { }
    for i = 1, #s, 2 do t[#t + 1] = string.char(tonumber(s:sub(i, i + 1), 16)) end
    ...
    for line in f:lines() do
        ...
        if k then
            if k == _H "63757272656e745f73656564" then
                ...
                local s1 = k:match("^" .. _H "6465617468735f" .. "(%-?%d+)$");
                if s1 then st.deaths[tonumber(s1)] = tonumber(v) or 0 end
                local s2 = k:match("^" .. _H "70656e64696e675f" .. "(%-?%d+)$");
                if s2 then st.pending[tonumber(s2)] = true end
            end
        end
    end
    f:close(); return st
...
local function _C(contents)
    ensure_dir_once()
    if contents == _last_written then return end
...
local function _D(st)
    ...
    local seeds = { };
    for seed in pairs(st.deaths) do seeds[#seeds + 1] = seed end;
    table.sort(seeds)
...
function SeedDeaths.get_current()
    local st = _B();
    if st.current_seed then return st.current_seed, st.deaths[st.current_seed] or 0 end
    return nil, nil
end

function SeedDeaths.get_for(seed)
    local st = _B(); seed = tonumber(seed);
...
function checkMapCompletition()
    ...
    if (Map.IndoorOrOutdoor == 1 and mapvars.monsterMap and mapvars.completed == nil) or(Map.IndoorOrOutdoor == 2 and mapvars.completed == nil) then
        if Map.Name == "d42.blv" then return end
        -- arena
...
function checkPityProtectedBoss(seed, chanceMult, generatedByBroodlord)
    if not vars.insanityMode then
        return nil, nil, nil
        -- No pity protection outside insanityMode
    end
    ...
    if broodlordChance < pityBroodlordChance and not generatedByBroodlord then
        ...
        hpMult = 2
        dmgMult = 1.5
        vars.broodlordPityCounter = 0
        -- Reset counter on success
    else
        vars.broodlordPityCounter = vars.broodlordPityCounter + 1
    ...
    if not skill and omnipotentChance < pityOmnipotentChance then
        ...
        hpMult = 4
        dmgMult = 2
        vars.omnipotentPityCounter = 0
        -- Reset counter on success
    elseif not skill then
        vars.omnipotentPityCounter = vars.omnipotentPityCounter + 1
        -- Increment on failure
    end
...
function events.GameInitialized2()
    -- to make the after all the other code
    function events.CalcDamageToPlayer(t)
            ...
            if t.Player then
                ...
                if skill == "Reflecting" or skill == "Omnipotent" then
                    if t.DamageKind ~= 4 then
                        ...
                        if damageKind == 50 then
                            -- transform dragon damage into energy
                            damageKind = 12
                        end
                        reflectedDamage = true
            ...
            if mon then
                ...
                if skill == "Leecher" or skill == "Omnipotent" then
                    ...
                    if (distance or 1e9) < 1500 and(mon.HP or 0) > 0 and mon.AIState ~= 19 and mon.AIState ~= 11 and mon.AIState ~= 5 then
                        ...
                        local timeMultiplier = Game.TurnBased and 4 or 1
                        -- nerf conservé
                        for pi = 0, Party.High do
                            ...
                            if pl then
                                ...
                                if (pl.SP or 0) >= 0 then
                                    local drainSPbase =(pl.SP or 0)
                                    -- si tu préfères le full SP: pl:GetFullSP()
...
function events.MonsterKilled(mon)
    ...
    -- reset experience
    local lvlBase = math.max(basetable[id].Level, totalLevel[id] / 3)
    -- added totalLevel/3 because of mapping
    local lvlBase = math.min(lvlBase, 100)
        ...
        for i = 0, Map.Monsters.High do
            ...
            if mon.AIState == 0 and mon.ShowOnMap and getDistance(mon.X, mon.Y, mon.Z) < 350 then
                ...
                if requiresHandling then
                    ...
                    -- atan2 gives angle in range -pi to pi

                    -- Calculate theoretical positions
                    local clockwise_angle = angle - math.pi / 2
                    -- Rotate clockwise
                    local counterclockwise_angle = angle + math.pi / 2
                    ...
                    -- Set velocity
                    local speed = mon.Velocity
                    -- This is the monster's movement speed
...
function events.CalcDamageToMonster(t)
    ...
    if data and data.Monster then
        ...
        local damage = getMonsterDamage(mon) / 3
        -- 1/3 of damage
