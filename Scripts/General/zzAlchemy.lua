...
function events.MonsterKilled(mon)
	...
	local sageGemMx=GetPartyCraftGemsSageCraftMx()
	local sageSpecialMx=GetPartyCraftSpecialSageCraftMx()
	
	bonusRoll=bonusRoll*sageGemMx
	...
	if baseCraftDrop then
		...
		DebugAddCombatLog("Crafting drop: " .. StrColor(0,255,255, Game.ItemsTxt[crafMaterialNumber].Name ))
		sdgPlaySound("chord.wav")
	end
	...
	for i=1061,1068 do
		...
		local pityAdjustedChance = pityAdjustedChance * bonusRoll*sageSpecialMx
		...
		if math.random() < pityAdjustedChance then
			...
			DebugAddCombatLog("Crafting drop: " .. StrColor(0,255,255, Game.ItemsTxt[i].Name ))
