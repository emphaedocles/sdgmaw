...
static const int DEFAULT_W = 256;
static const int DEFAULT_H = 276;
...
static void SaveWindowPlacementToRegistry(HWND hwnd)
{
	...
	if (RegCreateKeyExW(HKEY_CURRENT_USER, REG_KEY, 0, nullptr, 0, KEY_WRITE, nullptr, &hk, nullptr) == ERROR_SUCCESS)
	{
		...
		//RegSetValueExW(hk, L"Width", 0, REG_DWORD, (const BYTE*)&width, sizeof(width));
		//RegSetValueExW(hk, L"Height", 0, REG_DWORD, (const BYTE*)&height, sizeof(height));
		RegCloseKey(hk);
	}
}
static bool LoadWindowPlacementFromRegistry(int& outLeft, int& outTop, int& outW, int& outH)
{
	...
	//if (RegQueryValueExW(hk, L"Width", nullptr, &type, (LPBYTE)&data, &len) == ERROR_SUCCESS && type == REG_DWORD) outW = (int)data;
	//if (RegQueryValueExW(hk, L"Height", nullptr, &type, (LPBYTE)&data, &len) == ERROR_SUCCESS && type == REG_DWORD) outH = (int)data;
	RegCloseKey(hk);
	// consider valid if width/height non-zero
...
void MobRadar::AddEntity(int id, float x, float y, int tier)
{
	if (!s_hwnd) return;
...
static LRESULT CALLBACK MobRadar_WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		if (s_gdiplusToken)
		{
			...
			float partyangle = (std::numbers::pi/2.0f- _partyDirX)*r2d;// +std::numbers::pi / 2.0f)* r2d;// atan2(_partyDirY, _partyDirX);
			...
			//mx.RotateAt(angle, PointF(width / 2.0f, height / 2.0f));
			mx.Scale(1.0f, -1.0f);
			...
			REAL cx = 0;// width * 0.5f;
			REAL cy = 0;// height * 0.5f;
			...
			for (auto& kv : s_entities)
			{
				...
				if (dr > 0) 
				{
					nx = (ent.x) / dr; //if (nx < 0.0f) nx = 0.0f; if (nx > 1.0f) nx = 1.0f;
					ny = (ent.y) / dr; //if (ny < 0.0f) ny = 0.0f; if (ny > 1.0f) ny = 1.0f;
				}
				...
				SolidBrush brushEnt(Color(255, 255, 100, 100));
				...
				{
					g.FillEllipse(&brushEnt, px - r, py - r, r * 2.0f, r * 2.0f);
				}
				if (count == 0)
				{
					g.SetTransform(&mxW);
					std::wostringstream mobZ;
					mobZ << L"m0: " << dr << " || " << mu << " || " << nx << " , " << ny << " || " << px << " , " << py;

					g.DrawString(mobZ.str().c_str(), -1, &font, PointF(0, height-20), &textBrush);
			...
			mobCount << count << L" Total Mobs In Range, " << countBoss << " Bosses in Range";

			g.DrawString(mobCount.str().c_str(), -1, &font, PointF(0, 22), &textBrush);
		
			//std::wostringstream partyDir;
			//partyDir << "Party Face: RADS " << _partyDirX << " ,  DEGS "  << (_partyDirX*r2d);
			//g.DrawString(partyDir.str().c_str(), -1, &font, PointF(0, 20), &textBrush);
